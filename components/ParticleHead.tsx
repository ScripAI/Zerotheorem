...
export default function ParticleHead() {
  ...
  useEffect(() => {
    ...
    let returnToCenterTween: gsap.core.Tween | null = null;
    ...
    function onDocumentMouseMove(event: MouseEvent) {
      ...
      // Reset mouse in window flag and kill any existing return animation
      if (!isMouseInWindow) {
        isMouseInWindow = true;
        if (returnToCenterTween) {
          returnToCenterTween.kill();
          returnToCenterTween = null;
        }
      }
    }
    ...
    function onDocumentMouseLeave() {
      ...
      // Create smooth animation to return to center
      if (returnToCenterTween) {
        returnToCenterTween.kill();
      }
      
      returnToCenterTween = gsap.to({ mouseX, mouseY }, {
        duration: 2, // 2 seconds to return to center
        ease: "power2.out",
        mouseX: 0,
        mouseY: 0,
        onUpdate: function() {
          mouseX = this.targets()[0].mouseX;
          mouseY = this.targets()[0].mouseY;
        },
        onComplete: function() {
          returnToCenterTween = null;
        }
